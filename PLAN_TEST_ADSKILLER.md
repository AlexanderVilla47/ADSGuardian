...
| AK-TC-04 | ✅ Completado | Ruteo `not_actionable` para vencido no elegible | API n8n remota (`workflowId=8mlwAxLtJVrwpLhi`) validada con metodologia deterministica estricta sobre una sola corrida manual posterior al mensaje "listo": `execution_id=596` | `execution_id=596` (manual, `retryOf=null`) cumple forcing del caso (`Meta - Precheck Estado Ad.statusCode=200`, `Normalizar HTTP Precheck.http_body.effective_status=PAUSED`), `Evaluar Precheck Meta.precheck_state=not_actionable`, `Rutear Estado Precheck` sale por rama `not_actionable` (`out2=1`) y `Meta - Pausar Ad` no se ejecuta (`runs=0`). Dictamen: `PASS` | Para AK-TC-04 debe cumplirse `precheck=200` + `effective_status=PAUSED`, rutear `not_actionable` y no ejecutar `Meta - Pausar Ad` |
...
| AK-TC-06 | ✅ Completado | Precheck Meta fallido corta flujo antes de pausar | API n8n remota (`workflowId=8mlwAxLtJVrwpLhi`) con corrida manual unica usada para veredicto: `execution_id=587` | `execution_id=587` (manual, `retryOf=null`) valida input forzado `Meta - Precheck Estado Ad.queryParameters=statusCode=404` y confirma flujo esperado: `Normalizar HTTP Precheck.retryable=false`, `Evaluar Precheck Meta.precheck_state=failed`, `Rutear Estado Precheck` por salida failed (`previousNodeOutput=3`), ejecucion de `Alerta Crítica - Precheck fallido` y `Meta - Pausar Ad` no ejecutado | Con precheck no recuperable, no debe pausar; debe emitir alerta critica de precheck |
| AK-TC-07 | ✅ Completado | Extension de contrato resetea preventiva 48h | API n8n remota (`workflowId=8mlwAxLtJVrwpLhi`) + workflow UI (`rpnGFPo0nDthwzdB`) con secuencia deterministica Run1/Run2/Run3: `execution_id=607` -> `610` -> `611` | Run1 (`607`) marca preventiva (`Notificado_Previo=TRUE`); Run2 (`610`) extiende y resetea (`Notificado_Previo=false`, `Fecha_Fin=2026-04-10`); Run3 (`611`) vuelve a entrar a preventiva y remarca (`Notificado_Previo=TRUE`, `Fecha_Notificado_Previo=2026-04-08T15:22:50.997-03:00`) | Tras extension, `Notificado_Previo` debe volver a `false` y habilitar nueva preventiva |
| AK-TC-08 | ✅ Completado | Ejecucion manual forzada queda trazada | API n8n remota (`workflowId=8mlwAxLtJVrwpLhi`) validada con metodologia deterministica estricta sobre una sola corrida manual posterior al mensaje "listo": `execution_id=598` | `execution_id=598` (manual, `retryOf=null`) alcanza nodos minimos de trazabilidad manual (`Trigger Manual On-Demand`, `Contexto Manual`, `Sheets - Leer Contratos`, `Clasificar Contratos (Activo / 48h / Vencido)`), con evidencia de contexto manual (`Contexto Manual.run_mode=manual`) y trazabilidad por correlacion (`Clasificar...correlation_id=ak-1775667285851-aiyi85`). Dictamen: `PASS` | Debe registrarse corrida manual con correlacion y evidencia de salida |
...
- Validacion estricta AK-TC-06 sobre ultima corrida manual posterior al mensaje "listo": `execution_id=587` (manual, `retryOf=null`, `startedAt=2026-04-08T15:53:24.216Z`).
- Corrida valida para AK-TC-06 por input forzado de precheck: en snapshot de workflow de la misma ejecucion, `Meta - Precheck Estado Ad.queryParameters` incluye `statusCode=404`.
- Resultado esperado confirmado en `execution_id=587`: `Meta - Precheck Estado Ad.statusCode=404`, `Normalizar HTTP Precheck.retryable=false`, `Evaluar Precheck Meta.precheck_state=failed`, `Rutear Estado Precheck` deriva a rama failed (`previousNodeOutput=3`), se ejecuta `Alerta Crítica - Precheck fallido.alert_type=EXPIRED_NOT_PAUSED_PRECHECK_FAILED`, y `Meta - Pausar Ad` no aparece en `runData` (no ejecutado).
- Veredicto deterministico final AK-TC-06: `PASS`.
- Validacion estricta AK-TC-04 sobre ultima corrida manual posterior al mensaje "listo": `execution_id=596` (manual, `retryOf=null`, `startedAt=2026-04-08T16:29:13.962Z`).
- Corrida valida por forcing de AK-TC-04 en `execution_id=596`: `Meta - Precheck Estado Ad.statusCode=200` y `Normalizar HTTP Precheck.http_body.effective_status=PAUSED`.
- Resultado esperado confirmado en `execution_id=596`: `Evaluar Precheck Meta.precheck_state=not_actionable`, `Rutear Estado Precheck.out2=1` (rama `not_actionable`), y `Meta - Pausar Ad` no aparece en `runData` (`runs=0`).
- Veredicto deterministico final AK-TC-04: `PASS`.
- Cleanup post-test AK-TC-04 aplicado a baseline: `Meta - Precheck Estado Ad` restaurado a `queryParameters=[name={{$json.Ad_Name}}]` (sin `statusCode` ni `effective_status` forzados); `Meta - Pausar Ad` se mantiene baseline sin forcing; export local sincronizado en `workflows/contract-guard-daily-killswitch.json`.
- Validacion estricta AK-TC-08 sobre ultima corrida manual posterior al mensaje "listo": `execution_id=598` (manual, `retryOf=null`, `startedAt=2026-04-08T16:54:45.212Z`).
- Nodos minimos de trazabilidad manual confirmados en `execution_id=598`: `Trigger Manual On-Demand`, `Contexto Manual`, `Sheets - Leer Contratos`, `Clasificar Contratos (Activo / 48h / Vencido)`.
- Evidencia de trazabilidad de contexto manual en `execution_id=598`: `Contexto Manual.run_mode=manual` y `Clasificar Contratos...correlation_id=ak-1775667285851-aiyi85`.
- Veredicto deterministico final AK-TC-08: `PASS`.
- Validacion estricta AK-TC-07 Run3 sobre ultima corrida manual posterior al mensaje "listo run3": `execution_id=611` (manual, `retryOf=null`, `startedAt=2026-04-08T18:22:48.523Z`).
- Nodos alcanzados en `execution_id=611`: `Trigger Manual On-Demand`, `Contexto Manual`, `Sheets - Leer Contratos`, `Clasificar Contratos (Activo / 48h / Vencido)`, `Expand Preventiva`, `Payload Alerta Preventiva`, `Emitir Alerta Operativa`, `Merge`, `Sheets - Marcar Notificado_Previo`.
- Evidencia Run1 (`execution_id=607`): `AKTC07-RESET-001` entra a preventiva y se marca `Notificado_Previo=TRUE` con `Fecha_Notificado_Previo=2026-04-08T15:11:35.432-03:00`.
- Evidencia Run2 (`execution_id=610`, workflow UI `rpnGFPo0nDthwzdB`): extension de `Fecha_Fin=2026-04-09` a `2026-04-10` y reset de preventiva (`Notificado_Previo=false`) para `AKTC07-RESET-001`.
- Evidencia Run3 (`execution_id=611`): lectura/clasificacion muestran `AKTC07-RESET-001` con `Notificado_Previo=false`; vuelve a preventiva (`alert_type=PREVENTIVE_48H`) y `Sheets - Marcar Notificado_Previo` persiste `Notificado_Previo=TRUE` con `Fecha_Notificado_Previo=2026-04-08T15:22:50.997-03:00`.
- Cleanup post-test AK-TC-07 aplicado por restauracion de estado inicial (sin borrado de fila): webhook UI `execution_id=614` deja `AKTC07-RESET-001` con `Fecha_Fin=2026-04-09` y `Notificado_Previo=false`.
- Veredicto deterministico final AK-TC-07: `PASS`.
...
- Resuelto: evidencia de extension que resetea preventiva (`AK-TC-07`) cerrada con secuencia Run1/Run2/Run3 y before/after en Sheets.
...
| 2026-04-08 | AK-TC-06 validacion estricta: PASS + cleanup baseline | Se usa exclusivamente la ultima corrida manual posterior al mensaje "listo": `execution_id=587` (`startedAt=2026-04-08T15:53:24.216Z`, `retryOf=null`). Corrida valida por input forzado `precheck statusCode=404`; resultado esperado confirmado (`retryable=false`, `precheck_state=failed`, rama failed, alerta critica de precheck, sin ejecucion de `Meta - Pausar Ad`). Luego se aplica cleanup post-test a baseline: `Meta - Precheck Estado Ad` sin `statusCode` forzado (queda solo `name={{$json.Ad_Name}}`), `Meta - Pausar Ad` sin query params/sin `sendQuery`; export sincronizado en `workflows/contract-guard-daily-killswitch.json`. |
| 2026-04-08 | AK-TC-04 validacion estricta: FAIL + cleanup baseline | Se usa exclusivamente la ultima corrida manual posterior al mensaje "listo": `execution_id=590` (`startedAt=2026-04-08T16:07:24.859Z`, `retryOf=null`). No cumple forcing del caso (`precheck statusCode=200` OK pero `effective_status=ACTIVE`, esperado `PAUSED`); flujo observado en `ready_to_pause` (`Evaluar Precheck Meta.precheck_state=ready_to_pause`, `Rutear Estado Precheck.out0=1`) con ejecucion de `Meta - Pausar Ad` (`runs=1`). Se verifica/aplica cleanup baseline en remoto (`Meta - Precheck Estado Ad` solo `name={{$json.Ad_Name}}`; `Meta - Pausar Ad` sin forcing) y se sincroniza export local en `workflows/contract-guard-daily-killswitch.json`. |
| 2026-04-08 | AK-TC-04 validacion estricta final: PASS + cleanup baseline | Se usa exclusivamente la ultima corrida manual posterior al mensaje "listo": `execution_id=596` (`startedAt=2026-04-08T16:29:13.962Z`, `retryOf=null`). Corrida valida por forcing (`precheck statusCode=200` y `effective_status=PAUSED`) y resultado esperado confirmado (`Evaluar Precheck Meta.precheck_state=not_actionable`, `Rutear Estado Precheck.out2=1`, `Meta - Pausar Ad` no ejecutado). Cleanup aplicado en remoto a baseline (`Meta - Precheck Estado Ad` solo `name={{$json.Ad_Name}}`; `Meta - Pausar Ad` sin forcing) con export local sincronizado en `workflows/contract-guard-daily-killswitch.json`. |
| 2026-04-08 | AK-TC-08 validacion estricta: PASS | Se usa exclusivamente la ultima corrida manual posterior al mensaje "listo": `execution_id=598` (`startedAt=2026-04-08T16:54:45.212Z`, `retryOf=null`). Se confirman nodos minimos de trazabilidad manual (`Trigger Manual On-Demand`, `Contexto Manual`, `Sheets - Leer Contratos`, `Clasificar Contratos (Activo / 48h / Vencido)`), evidencia de contexto manual (`Contexto Manual.run_mode=manual`) y correlacion operativa (`Clasificar...correlation_id=ak-1775667285851-aiyi85`). Dictamen final: `PASS`. |
| 2026-04-08 | AK-TC-07 validacion estricta Run3: PASS + cleanup fila de prueba | Regla deterministica aplicada sobre una sola corrida manual posterior al mensaje "listo run3": `execution_id=611` (`startedAt=2026-04-08T18:22:48.523Z`, `retryOf=null`). Para `AKTC07-RESET-001`, se confirma lectura/clasificacion, entrada a preventiva (`alert_type=PREVENTIVE_48H`) y marcado en Sheets (`Notificado_Previo=TRUE`, `Fecha_Notificado_Previo=2026-04-08T15:22:50.997-03:00`). Se cierra AK-TC-07 en PASS con secuencia Run1/Run2/Run3 (`607` -> `610` -> `611`). Cleanup aplicado por restauracion (sin borrar fila) via workflow UI: `execution_id=614`, `Fecha_Fin=2026-04-09`, `Notificado_Previo=false`. |
